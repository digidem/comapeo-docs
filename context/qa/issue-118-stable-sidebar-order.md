...
Verify that a _partial_ Notion sync (processing only a subset of pages) does **not** reshuffle:
...
Expected: either no diff, or only diffs attributable to _newly generated_ files/pages in the smaller run (not re-numbering existing pages).
