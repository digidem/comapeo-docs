...
describe("ContentGenerator", () => {
  describe("detectContentType", () => {
    it("should detect tutorial content type", () => {
      expect(ContentGenerator.detectContentType("Getting Started Guide")).toBe(
        "tutorial"
      );
    ...
    it("should detect reference content type", () => {
      ...
      expect(ContentGenerator.detectContentType("Command Line Interface")).toBe(
        "reference"
      );
      expect(ContentGenerator.detectContentType("Settings Documentation")).toBe(
        "reference"
      );
    });
    ...
    it("should detect troubleshooting content type", () => {
      ...
      expect(
        ContentGenerator.detectContentType("FAQ - Frequently Asked Questions")
      ).toBe("troubleshooting");
      expect(
        ContentGenerator.detectContentType("Error Messages and Solutions")
      ).toBe("troubleshooting");
      expect(
    ...
    it("should handle case insensitive matching", () => {
      ...
      expect(ContentGenerator.detectContentType("TrOuBlEsHoOtInG")).toBe(
        "troubleshooting"
      );
    });
  });
  ...
  describe("generateCompletePage", () => {
    it("should generate intro content with all required blocks", () => {
      ...
      expect(blocks[0].heading_1?.rich_text[0]?.plain_text).toBe(
        "CoMapeo Overview"
      );
    ...
    it("should generate tutorial content with structured elements", () => {
      ...
      const numberedLists = blocks.filter(
    ...
    it("should generate troubleshooting content with bullet points", () => {
      ...
      const bulletedLists = blocks.filter(
      ...
      blocks.forEach((block) => {
        ...
        switch (block.type) {
          ...
          case "bulleted_list_item":
            ...
            expect(Array.isArray(block.bulleted_list_item?.rich_text)).toBe(
              true
            );
            break;
          case "numbered_list_item":
            expect(block.numbered_list_item?.rich_text).toBeDefined();
            expect(Array.isArray(block.numbered_list_item?.rich_text)).toBe(
              true
            );
            break;
          case "code":
    ...
    it("should include contextual CoMapeo content", () => {
      ...
      const hasCoMapeoContent = coMapeoTerms.some((term) =>
