...
describe("notion-placeholders", () => {
  ...
  const mockBlocks = [
    {
      type: "heading_1",
      heading_1: { rich_text: [{ plain_text: "Getting Started" }] },
    },
    {
      type: "paragraph",
      paragraph: {
        rich_text: [{ plain_text: "This is placeholder content." }],
      },
    },
  ];
  ...
  describe("main function", () => {
    ...
    it("should handle errors gracefully", async () => {
      vi.mocked(fetchNotionData.fetchNotionData).mockRejectedValue(
        new Error("API Error")
      );
  ...
  describe("error handling", () => {
    it("should handle analysis errors", async () => {
      vi.mocked(fetchNotionData.fetchNotionData).mockResolvedValue(mockPages);
      vi.mocked(PageAnalyzer.analyzePages).mockRejectedValue(
