 ...
 */
function detectFormatFromBuffer(buf: Buffer): 'jpeg' | 'png' | 'webp' | 'svg' | 'gif' | 'unknown' {
...
export async function compressImage(inputBuffer: Buffer) {
  try {
    ...
    switch (format) {
      ...
      default:
        return { compressedBuffer: inputBuffer, originalSize: inputBuffer.length, compressedSize: inputBuffer.length };
    }
