...
export class StatusAnalyzer {
  ...
  static analyzePublicationStatus(pages: PageWithStatus[]): {
    ...
    console.log(
      `✅ Analysis complete: ${readiness.readinessPercentage}% ready for publication`
    );
   ...
   */
  private static generateStatusBreakdown(
    ...
    for (const [status, statusPages] of statusGroups.entries()) {
      breakdown.push({
        ...
        percentage:
          totalPages > 0
            ? Math.round((statusPages.length / totalPages) * 100)
            : 0,
   ...
   */
  private static assessPublicationReadiness(
    pages: PageWithStatus[]
  ): PublicationReadiness {
    const readyToPublish = pages.filter(
      (page) => page.status === "Ready to publish"
    ).length;
    const totalPages = pages.length;
    const needsWork = totalPages - readyToPublish;
    const readinessPercentage =
      totalPages > 0 ? Math.round((readyToPublish / totalPages) * 100) : 0;
    ...
    const draftPages = pages.filter(
      (page) => page.status === "Draft" || page.status === "In progress"
    );
    ...
    const englishPages = pages.filter(
      (page) => !page.language || page.language === "English"
    );
    ...
    const outdatedPages = pages.filter(
      (page) =>
   ...
   */
  private static analyzeLanguageProgress(
    pages: PageWithStatus[]
  ): LanguageAnalysis[] {
    ...
    for (const [language, languagePages] of languageGroups.entries()) {
      ...
      const readyPages = languagePages.filter(
        (p) => p.status === "Ready to publish"
      ).length;
      const draftPages = languagePages.filter(
      ...
      const lastUpdated = new Date(
        Math.max(...languagePages.map((p) => p.lastEdited.getTime()))
      );
  ...
  private static analyzeTrends(pages: PageWithStatus[]) {
    ...
    const recentlyUpdated = pages.filter(
      (page) => page.lastEdited > sevenDaysAgo
    ).length;
    const staleContent = pages.filter(
      (page) => page.lastEdited < thirtyDaysAgo
    ).length;
  ...
  static identifyContentGaps(pages: PageWithStatus[]): ContentGaps {
    ...
    console.log(
      `📝 Found ${missingPages.length} missing pages, ${inconsistentStructure.length} structure issues`
    );
   ...
   */
  private static identifyMissingPages(
    pages: PageWithStatus[]
  ): ContentGaps["missingPages"] {
    ...
    for (const englishPage of englishPages) {
      for (const lang of languages) {
        const hasTranslation = pages.some(
          (p) =>
            p.language === lang &&
   ...
   */
  private static identifyStructureIssues(
    pages: PageWithStatus[]
  ): ContentGaps["inconsistentStructure"] {
   ...
   */
  private static identifyOutdatedContent(
    pages: PageWithStatus[]
  ): ContentGaps["outdatedContent"] {
    const sixtyDaysAgo = new Date(Date.now() - 60 * 24 * 60 * 60 * 1000);

    return pages
      .filter(
        (page) =>
          page.lastEdited < sixtyDaysAgo && page.status === "Ready to publish"
      )
      .map((page) => ({
        pageId: page.id,
        title: page.title,
        ...
        staleDays: Math.floor(
          (Date.now() - page.lastEdited.getTime()) / (24 * 60 * 60 * 1000)
        ),
      }))
  ...
  static generateReadinessReport(pages: PageWithStatus[]): {
    ...
    if (gaps.missingPages.length > 0) {
      ...
      const highPriority = gaps.missingPages.filter(
        (p) => p.priority === "high"
      );
      for (const missing of highPriority.slice(0, 5)) {
        recommendations.push(
          `   - ${missing.expectedTitle} (${missing.reason})`
        );
      }
    }
    ...
    if (analysis.trends.staleContent > 0) {
      recommendations.push(
        `🔄 **Update Stale Content:** ${analysis.trends.staleContent} pages older than 30 days`
      );
    }
