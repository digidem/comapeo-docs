...
describe('notionClient', () => {
  ...
  describe('module initialization', () => {
    it('should throw error when NOTION_API_KEY is not defined', async () => {
      ...
      }).rejects.toThrow('NOTION_API_KEY is not defined in the environment variables.');
    });
    ...
    it('should throw error when DATABASE_ID is not defined', async () => {
      ...
      await expect(async () => {
        await import('./notionClient');
      }).rejects.toThrow('DATABASE_ID is not defined in the environment variables.');
    });
  ...
  describe('enhancedNotion.databasesQuery', () => {
    ...
    it('should retry on network error', async () => {
      // Arrange
      const networkError = createMockError('Network error', undefined, 'ECONNABORTED');
      const successData = { results: [], has_more: false };
      const queryParams = { database_id: 'test-db' };
    ...
    it('should not retry on client error (400)', async () => {
      ...
      // Act & Assert
      await expect(enhancedNotion.databasesQuery(queryParams))
  ...
  describe('enhancedNotion.blocksChildrenList', () => {
    it('should return blocks data on successful request', async () => {
      ...
      expect(mockClient.blocks.children.list).toHaveBeenCalledWith(blocksParams);
    });
  ...
  describe('retry logic utilities', () => {
    it('should calculate exponential backoff delays correctly', async () => {
      // This test verifies the delay calculation indirectly through retry behavior
      // Arrange
      const rateLimitError = createMockError('Rate limited', 429);
      const queryParams = { database_id: 'test-db' };
      ...
      // Should have taken some time due to delays (at least 1000ms for first retry)
      expect(totalTime).toBeGreaterThan(500); // Allow some margin for test execution
      expect(mockClient.databases.query).toHaveBeenCalledTimes(4);
    });
  });
