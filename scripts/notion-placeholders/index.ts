...
import { ContentGenerator, ContentGenerationOptions } from './contentGenerator.js';
...
const printHelp = () => {
  ...
  console.log('Generates meaningful placeholder content for empty Notion pages.\n');
  console.log(chalk.bold('Usage:'));
  console.log('  npm run notion:gen-placeholders [options]\n');
  console.log(chalk.bold('Options:'));
  console.log('  --dry-run, -d              Preview changes without modifying Notion');
  console.log('  --verbose, -v              Show detailed output');
  console.log('  --content-length <length>  Content length: short, medium, long (default: medium)');
  console.log('  --no-skip-recent           Process recently modified pages');
  console.log('  --recent-hours <hours>     Hours to consider "recent" (default: 24)');
  console.log('  --force                    Force update even if page has content');
  console.log('  --no-backup                Skip creating backups');
  console.log('  --include-removed          Include pages with "Remove" status');
  console.log('  --filter-status <status>   Only process pages with specific status');
  ...
  console.log('  npm run notion:gen-placeholders --content-length long --verbose');
  console.log('  npm run notion:gen-placeholders --filter-status "Draft" --max-pages 10');
};
...
async function main() {
  ...
  if (options.dryRun) {
    console.log(chalk.yellow('🔍 DRY RUN MODE - No changes will be made to Notion\n'));
  }
  ...
  try {
    ...
    const filteredPages = pages.filter(page => {
      const elementType = page.properties?.[NOTION_PROPERTIES.ELEMENT_TYPE]?.select?.name || page.properties?.["Section"]?.select?.name;
      const status = page.properties?.[NOTION_PROPERTIES.STATUS]?.select?.name || page.properties?.["Status"]?.select?.name;
    ...
    const pagesToProcess = options.maxPages ? filteredPages.slice(0, options.maxPages) : filteredPages;
    
    console.log(chalk.blue(`📊 Analyzing ${pagesToProcess.length} pages for content gaps (filtered from ${pages.length} total)...\n`));
    ...
    const pageAnalyses = await PageAnalyzer.analyzePages(
      pagesToProcess.map(page => ({
        id: page.id,
        title: page.properties?.[NOTION_PROPERTIES.TITLE]?.title?.[0]?.plain_text || 
    ...
    console.log(`  Pages needing enhancement: ${summary.pagesNeedingEnhancement}`);
    console.log(`  Average content score: ${summary.averageContentScore.toFixed(1)}/100`);
    console.log(`  Recently modified (skipped): ${summary.recentlyModifiedSkipped}`);

    // Generate content for pages that need it
    const pagesToUpdate = Array.from(pageAnalyses.entries())
      .filter(([, analysis]) => analysis.recommendedAction === 'fill' || 
    ...
    for (let i = 0; i < pagesToUpdate.length; i++) {
      ...
      console.log(chalk.cyan(`📝 [${i + 1}/${pagesToUpdate.length}] Generating content for: "${title}"`));
      console.log(chalk.gray(`   Type: ${analysis.recommendedContentType} | Score: ${analysis.contentScore}/100`));
    ...
    if (updateSummary.errors.length > 0) {
      ...
      updateSummary.errors.forEach(error => console.log(chalk.red(`  ${error}`)));
    }
    ...
    if (options.verbose) {
      ...
      console.log(chalk.gray(`  Storage used: ${(backupStats.totalSizeBytes / 1024 / 1024).toFixed(2)} MB`));
    }
    ...
    if (updateSummary.successfulUpdates > 0) {
      ...
      console.log(chalk.gray(`   Total content blocks generated: ${updateSummary.totalBlocksAdded}`));
    }
