...
async function main() {
  ...
  try {
    ...
    let data: Array<Record<string, unknown>> = (await fetchNotionData(
      filter
    )) as Array<Record<string, unknown>>;
    // Sort data by Order property if available to ensure proper sequencing
    data = (await sortAndExpandNotionData(data)) as Array<
    ...
    const { totalSaved, sectionCount, titleSectionCount } =
      await generateBlocks(data, (progress) => {
        generateSpinner.text = chalk.blue(
          `Generating blocks: ${progress.current}/${progress.total}`
        );
      });
    ...
    console.log(
      chalk.bold.cyan(
        `A total of ${(totalSaved / 1024).toFixed(2)} KB was saved on image compression.`
      )
    );
    console.log(
      chalk.bold.yellow(
        `Created ${sectionCount} section folders with _category_.json files.`
      )
    );
    console.log(
      chalk.bold.magenta(
        `Applied ${titleSectionCount} title sections to content items.`
      )
    );
