...
export function sanitizeMarkdownContent(content: string): string {
  ...
  for (let i = 0; i < 5 && /\{[^{}]*\}/.test(content); i++) {
    content = content.replace(/\{([^{}]*)\}/g, (_m, inner) => String(inner).trim());
  }
  ...
  content = content.replace(/__CODEBLOCK_(\d+)__/g, (_m, i) => codeBlocks[Number(i)]);
  content = content.replace(/__CODESPAN_(\d+)__/g, (_m, i) => codeSpans[Number(i)]);
