...
import { fetchAllNotionData, groupPagesByStatus, groupPagesByElementType, buildPageHierarchy, filterPages } from './fetchAll.js';
...
describe('fetchAll', () => {
  ...
  describe('fetchAllNotionData', () => {
    ...
    it('should exclude archived pages when includeArchived is false', async () => {
      ...
      await fetchAllNotionData({ includeArchived: false, includeRemoved: true });
    ...
    it('should handle sub-page fetch errors gracefully', async () => {
      ...
      vi.mocked(enhancedNotion.pagesRetrieve).mockRejectedValue(new Error('Sub-page not found'));
    ...
    it('should sort pages correctly', async () => {
      const unsortedPages = [
        { ...mockNotionPages[1], properties: { ...mockNotionPages[1].properties, Order: { number: 3 } } },
        { ...mockNotionPages[0], properties: { ...mockNotionPages[0].properties, Order: { number: 1 } } }
      ];
    ...
    it('should handle API errors', async () => {
      vi.mocked(enhancedNotion.databasesQuery).mockRejectedValue(new Error('API Error'));
  ...
  describe('filterPages', () => {
    ...
    it('should filter by date range', () => {
      ...
      expect(recentPages[0].lastEdited.getTime()).toBeGreaterThan(new Date('2024-01-10').getTime());
      expect(oldPages).toHaveLength(1);
      expect(oldPages[0].lastEdited.getTime()).toBeLessThan(new Date('2024-01-10').getTime());
    });
