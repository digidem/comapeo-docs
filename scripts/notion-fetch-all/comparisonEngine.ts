...
export class ComparisonEngine {
  ...
  static async compareWithPublished(
    ...
    // Analyze differences
    const differences = this.analyzeDifferences(previewPages, publishedStructure);
    
    // Calculate impact
    const impact = this.calculateImpact(previewSections, publishedStructure, differences);
    ...
    const result: ComparisonResult = {
      ...
      preview: {
        ...
        languages: [...new Set(previewPages.map(p => p.language).filter(Boolean))]
      },
      differences,
      impact
    };

    console.log(`✅ Comparison complete: ${differences.newPages.length} new pages, ${differences.updatedPages.length} updates`);
  ...
  private static analyzeDifferences(
    ...
    const newPages = previewPages
      .filter(page => !publishedTitles.has(page.title) && page.status === 'Ready to publish')
      .map(page => ({
        title: page.title,
        status: page.status,
  ...
  private static calculateImpact(
    ...
    const sidebarChanges = this.analyzeSidebarChanges(previewSections, publishedStructure);

    // Calculate content volume changes
    const currentPages = publishedStructure.metadata.totalPages;
    const newPages = differences.newPages.length - differences.removedPages.length;
    const contentVolume = {
      increase: newPages,
    ...
    const structuralChanges = sidebarChanges.filter(
      change => change.type === 'added' || change.type === 'moved' || change.type === 'removed'
    ).length;
  ...
  private static analyzeSidebarChanges(
    ...
    const publishedSectionTitles = new Set(publishedStructure.sections.map(s => s.title));
    ...
    const publishedStructureHash = this.generateStructureHash(publishedStructure.sections);
   ...
   */
  private static findSectionForPage(page: PageWithStatus, allPages: PageWithStatus[]): string {
  ...
  static generateMigrationChecklist(comparison: ComparisonResult): {
    ...
    if (comparison.differences.newPages.length > 5) {
      checklist.preDeployment.push('✅ Extra review for large content additions');
    }
