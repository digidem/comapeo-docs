...
function printHelp(): void {
  ...
  console.log('Exports complete Notion database with comprehensive block-level content analysis.\\n');
  console.log(chalk.bold('Usage:'));
  console.log('  npm run notion:export [options]\\n');
  console.log(chalk.bold('Options:'));
  console.log('  --verbose, -v           Show detailed progress information');
  console.log('  --quick, -q             Skip detailed content analysis (faster)');
  console.log('  --output-prefix, -o     Custom prefix for output files');
  console.log('  --max-pages <number>    Limit number of pages to process (for testing)');
  console.log('  --status-filter <name>  Only export pages with specific status');
  console.log('  --no-raw-data          Exclude raw page data from export (smaller files)');
...
function getOutputPaths(options: ExportOptions) {
  ...
  return {
    complete: path.resolve(process.cwd(), `${prefix}_db_complete_${timestamp}.json`),
    analysis: path.resolve(process.cwd(), `${prefix}_content_analysis_${timestamp}.json`)
  };
}
...
function extractTextFromBlock(block: Record<string, any>): string {
  ...
  switch (blockType) {
    case 'image':
      return blockContent.caption?.map((c: any) => c.plain_text || '').join('') || '[Image]';
    case 'video':
      return blockContent.caption?.map((c: any) => c.plain_text || '').join('') || '[Video]';
    case 'file':
      return blockContent.caption?.map((c: any) => c.plain_text || '').join('') || '[File]';
    case 'bookmark':
      return blockContent.url || '[Bookmark]';
    case 'equation':
      return blockContent.expression || '[Equation]';
    case 'code':
      return blockContent.rich_text?.map((t: any) => t.plain_text || '').join('') || '[Code]';
    case 'divider':
      return '[Divider]';
...
function analyzeBlock(block: Record<string, any>): BlockAnalysis {
  ...
  const hasChildren = block.has_children || (block.children && block.children.length > 0);
 ...
 */
function analyzePageStructure(blocks: BlockAnalysis[]): PageAnalysis['structure'] {
  const structure = {
 ...
 */
function analyzePage(page: Record<string, any>, blocks: Record<string, any>[]): PageAnalysis {
  ...
  // Calculate metrics
  const totalTextLength = blockAnalyses.reduce((sum, block) => sum + block.contentLength, 0);
  const contentScore = calculateContentScore(blockAnalyses);
  const hasContent = blockAnalyses.some(block => block.hasContent) || totalTextLength > 20;
...
export async function exportNotionDatabase(cliOptions?: ExportOptions): Promise<void> {
  const options = cliOptions || parseCliArgs();
  const outputPaths = getOutputPaths(options);
  
  console.log(chalk.bold.cyan("🚀 CoMapeo Notion Complete Export & Analysis\n"));
  
  if (options.verbose) {
    console.log(chalk.gray(`Options: ${JSON.stringify(options, null, 2)}`));
    console.log(chalk.gray(`Output files: ${JSON.stringify(outputPaths, null, 2)}\n`));
  }
  ...
  try {
    ...
    if (options.statusFilter) {
      allPages = allPages.filter(page => {
        const status = page?.properties?.[NOTION_PROPERTIES.STATUS]?.select?.name ?? null;
        return status === options.statusFilter;
      });
      if (options.verbose) {
        console.log(chalk.blue(`Filtered to ${allPages.length} pages with status "${options.statusFilter}"`));
      }
    }
    ...
    spinner.succeed(chalk.green(`✅ Fetched ${allPages.length} pages from Notion`));
    ...
    for (const page of allPages) {
      ...
      try {
        ...
        if (options.verbose && processedCount % 5 === 0) {
          spinner.text = chalk.blue(`Fetching blocks: ${processedCount}/${allPages.length} pages processed`);
    ...
    spinner.succeed(chalk.green(`✅ Fetched blocks for ${allPages.length} pages`));

    // Step 3: Perform comprehensive analysis
    const analysisMessage = options.quick ? 'Performing basic content analysis...' : 'Performing comprehensive content analysis...';
    ...
    const averageContentScore = pageAnalyses.length > 0 ? totalContentScore / pageAnalyses.length : 0;
    const averageBlocksPerPage = pageAnalyses.length > 0 ? totalBlocks / pageAnalyses.length : 0;
    const averageTextLengthPerPage = pageAnalyses.length > 0 ? totalTextLength / pageAnalyses.length : 0;
    ...
    const serializedResult = {
      ...
      statistics: {
        ...
        statusBreakdown: Object.fromEntries(exportResult.statistics.statusBreakdown),
        elementTypeBreakdown: Object.fromEntries(exportResult.statistics.elementTypeBreakdown),
        languageBreakdown: Object.fromEntries(exportResult.statistics.languageBreakdown),
        blockTypeBreakdown: Object.fromEntries(exportResult.statistics.blockTypeBreakdown)
      },
    ...
    await writeFile(outputPaths.complete, JSON.stringify(serializedResult, null, 2), "utf8");
    ...
    const analysisSummary = {
      ...
      summary: {
        ...
        excellentContent: pageAnalyses.filter(p => p.contentScore >= 80).length
      },
    ...
    await writeFile(outputPaths.analysis, JSON.stringify(analysisSummary, null, 2), "utf8");
    ...
    console.log(chalk.gray(`Main export: ${path.relative(process.cwd(), outputPaths.complete)}`));
    console.log(chalk.gray(`Analysis summary: ${path.relative(process.cwd(), outputPaths.analysis)}`));

    console.log(chalk.bold('\n📊 Content Analysis Summary:'));
    console.log(`  Total Pages: ${chalk.cyan(allPages.length)}`);
    console.log(`  Empty Pages: ${chalk.red(emptyPages)} (${Math.round((emptyPages / allPages.length) * 100)}%)`);
    console.log(`  Contentful Pages: ${chalk.green(contentfulPages)} (${Math.round((contentfulPages / allPages.length) * 100)}%)`);
    console.log(`  Average Content Score: ${chalk.yellow(Math.round(averageContentScore * 10) / 10)}/100`);
    console.log(`  Ready to Publish: ${chalk.green(readyToPublish.length)} pages`);
    console.log(`  Total Blocks: ${chalk.cyan(totalBlocks.toLocaleString())}`);
    console.log(`  Total Text Length: ${chalk.cyan(totalTextLength.toLocaleString())} characters`);
    ...
    Array.from(blockTypeBreakdown.entries())
      ...
      .forEach(([blockType, count]) => {
        console.log(`  ${blockType}: ${chalk.cyan(count.toLocaleString())} blocks`);
      });
